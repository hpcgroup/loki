...
def get_s_hat_forward():
    def modified_forward(
        ...
        **kwargs,
        ...
        scaling_factor = dh * (torch.abs(key_states).sum(-1, keepdim=True) / key_scaling_states)

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(scaling_factor)
