...
from .h2o_opt_utils import local_heavy_hitter_mask as local_heavy_hitter_mask_opt
...
def get_h2o_forward(args):
    def modified_forward(
        ...
        **kwargs,
        ...
        # mask_bottom = ones
        attn_weights[~mask_bottom] = torch.min(causal_mask)
...
def get_h2o_opt_forward(args):
    def modified_forward(
        ...
        **kwargs,    
        ...
        alpha = None
        if heavy_budget > 0:
            mask_bottom = local_heavy_hitter_mask_opt(attn_weights, heavy_budget) # Default: No padding applied to input
        else:
            mask_bottom = torch.zeros_like(attn_weights, dtype=torch.bool)
...
def make_llama_attention_h2o(args):
    #TODO: For Llama 70B models, use the optimized version of the function

    if not args.lm_harness_eval and (
          args.model_id == "meta-llama/Llama-2-70b-hf"
          or args.model_id == "meta-llama/Meta-Llama-3-70B"
          or "Mixtral" in args.model_id
      ):
        print ("Modifying Llama Attention -> HF Optimised H2O")
        print (f"Heavy and Recent Ratio:{args.heavy_ratio}")
        LlamaAttention.forward = get_h2o_opt_forward(args)
    else:
