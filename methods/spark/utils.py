...
def mask_attn_spark(attn_weights, attention_mask, query_states, key_states, top_r, top_k, l=-1):
    ...
    i1 = torch.topk(torch.abs(key_states), top_r, -1).indices
    ...
    scaling_factor = head_dim * (torch.abs(key_states_sparse).sum(-1 , keepdim=True) / torch.abs(key_states).sum(-1, keepdim = True))
...
def test_spar_mask():
    ...
    spar_attn, alpha = mask_attn_spark(attention, attention_mask, q_test, k_test, 2, 2, -1)
    print (spar_attn)
